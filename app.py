...
# 2. سائیڈ بار لاگ ان پینل (خوبصورت ڈیزائن)
st.sidebar.markdown("<h2 style='text-align: center; color: #1E3A8A; font-family: Urbanist;'>🔒 INSTAPLAST<br>Login Panel</h2>", unsafe_allow_html=True)
st.sidebar.write("---")
...
# --- مین اسکرین ہیڈر (INSTAPLAST BEAUTIFUL HEADER) ---
st.markdown("""
    ...
    with col1:
        ...
        st.markdown("""
            <div style="text-align: center; margin-bottom: 15px; margin-top: 10px;">
                <img src="https://img.icons8.com/illustrations/lexicon/130/avatar.png" alt="Avatar" style="width:120px; background-color:#E2E8F0; border-radius:50%; border: 3px solid #0D9488; padding: 5px;">
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🚀 چھٹی کی درخواست سبمٹ کریں", use_container_width=True):
            if end_date >= start_date:
                ...
                if worker_data["balances"][b_key] >= days_requested:
                    ...
                    st.success(f"🎉 مبارک ہو! {days_requested} چھٹیاں کامیابی سے مائنس کر دی گئیں اور آپ کا ریکارڈ اپڈیٹ ہو گیا ہے۔")
                    st.balloons() # خوبصورت غبارے اڑیں گے چھٹی اپلائی ہونے پر
                    st.rerun()
                else:
...
# --- اگر ایڈمن لاگ ان ہے ---
    ...
    # کالمز بنا کر دونوں کاموں کو سامنے سامنے کر دیا تاکہ ڈھونڈنا آسان ہو
