...
# 1. ڈیٹا سیٹ کرنا (بشمول اپڈیٹڈ لیو کیٹیگریز)
                ...
                if end_date >= start_date:
                    ...
                    if selected_pay_type == "Without Pay (بغیر تنخواہ/Unpaid)":
                        ...
                        if worker_data["balances"].get(b_key, 0.0) >= days_requested:
                            ...
                            apply_attendance_dates()
                            st.success(f"🎉 {days_requested} چھٹیاں آپ کے بیلنس سے مائنس کر کے منتھ کارڈ اپڈیٹ کر دیا گیا ہے۔")
                            st.balloons()
                            st.rerun()
                        else:
    ...
    with tab1:
        ...
        with col_admin1:
            ...
            with st.form("add_form", clear_on_submit=True):
                ...
                st.markdown("<p style='color:#1E3A8A; font-weight:bold; margin-top:15px;'>📋 پورے سال کی الاؤ چھٹیاں (Yearly Leaves Allowance):</p>", unsafe_allow_html=True)
                col_l1, col_l2 = st.columns(2)
                allow_casual = col_l1.number_input("Casual Leave (ضروری کام کی چھٹیاں)", min_value=0.0, value=10.0)
                allow_sick = col_l2.number_input("Sick Leave (بیماری کی چھٹیاں)", min_value=0.0, value=8.0)
                allow_annual = col_l1.number_input("Annual Leave (سالانہ چھٹیاں)", min_value=0.0, value=14.0)
                allow_comp = col_l2.number_input("Compensatory Leave (متبادل چھٹیاں)", min_value=0.0, value=0.0)
